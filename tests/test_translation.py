...
def test_initialize_model_cuda_fallback():
    ...
    with patch('torch.cuda.is_available', return_value=False), \
        ...
        with patch('src.logging_setup.logger') as mock_logger:
            ...
            assert model._device == "cpu"
            mock_logger.warning.assert_called_with("CUDA requested but not available, falling back to CPU")
...
def test_translate(translation_model):
    ...
    translation_model.pipeline.assert_called_with(
        "Hello, how are you?",
        src_lang="en_XX",
        tgt_lang="fr_XX",
        max_length=200,
...
def test_translate_error(translation_model):
    ...
    translation_model.pipeline.side_effect = Exception("Translation error")
    ...
    assert "Translation error" in str(excinfo.value)
...
def test_get_model():
    ...
    with patch('src.models.translation.TranslationModel') as mock_cls:
