...
# Configuration
SERVICE_URL="http://lexi-shift.default.74.224.102.71.nip.io"
...
check_pods() {
    ...
    kubectl get pods -l serving.kserve.io/inferenceservice=lexi-shift -o wide
    echo -e "${BLUE}=== Pod Count ===${NC}"
    kubectl get pods -l serving.kserve.io/inferenceservice=lexi-shift --no-headers | wc -l | tr -d ' '
}
...
analyze_request_distribution() {
    ...
    # Get pod names
    pod1=$(kubectl get pods -l serving.kserve.io/inferenceservice=lexi-shift -o jsonpath='{.items[0].metadata.name}')
    pod2=$(kubectl get pods -l serving.kserve.io/inferenceservice=lexi-shift -o jsonpath='{.items[1].metadata.name}')
    ...
    pod1_logs=$(kubectl logs $pod1 --tail=40 | grep "Translation successful" | grep -o "request_id\": \"[^\"]*\"" | sed 's/request_id\": \"//' | sed 's/\"//')
    
    # Get logs from pod 2 and extract request IDs
    echo "Checking logs for Pod 2..."
    pod2_logs=$(kubectl logs $pod2 --tail=40 | grep "Translation successful" | grep -o "request_id\": \"[^\"]*\"" | sed 's/request_id\": \"//' | sed 's/\"//')
    ...
    echo -e "Pod 1 handled ${CYAN}$pod1_count${NC} requests"
    if [ $pod1_count -gt 0 ]; then
        echo -e "Request IDs: ${pod1_requests[*]}"
    fi
    
    echo -e "Pod 2 handled ${CYAN}$pod2_count${NC} requests"
    if [ $pod2_count -gt 0 ]; then
        echo -e "Request IDs: ${pod2_requests[*]}"
    fi
...
send_request() {
    local id=$1
    local text="This is test request ${id}. Testing translation service scaling."
    
    echo "Sending request $id..."
    curl -s -X POST "$SERVICE_URL/translate" \
         -H "Content-Type: application/json" \
         -d "{\"text\": \"$text\", \"options\": {\"source_lang\": \"en\", \"target_lang\": \"fr\"}}" \
         > /dev/null
...
while [ "$pods_ready" = false ]; do
    ...
    total_pods=$(kubectl get pods -l serving.kserve.io/inferenceservice=lexi-shift --no-headers | wc -l | tr -d ' ')
    
    # Get container readiness for all pods in service
    pod_status=$(kubectl get pods -l serving.kserve.io/inferenceservice=lexi-shift -o jsonpath='{range .items[*]}{range .status.containerStatuses[*]}{.ready}{" "}{end}{"\n"}{end}')
